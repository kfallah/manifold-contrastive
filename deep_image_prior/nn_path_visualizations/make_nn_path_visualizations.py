...
def compute_manifold_path_nearest_images(
    path_samples,
    embeddings,
):
    """
        Computes a path along the manifold for a given
        transport operator, and finds the images in 
        embedding space that are closes to those
        points along the path.
    """
    images = []
    nearest_neighbors = []
    # for each path sample find the index of the closest embedding to it
    for path_sample in path_samples:
        # Find the closest embedding to the path sample
        print(f"path_sample.shape: {path_sample.shape}")
        print(f"embeddings.feature_list.shape: {embeddings.feature_list.shape}")
        print(f"embeddings - path_sample: {torch.norm(embeddings.feature_list.cpu() - path_sample.cpu(), dim=1).shape}")
        closest_embedding_index = torch.argmin(
            torch.norm(embeddings.feature_list.cpu() - path_sample.cpu(), dim=1)
        )
        nearest_neighbors.append(
            embeddings.feature_list[closest_embedding_index].detach().cpu().numpy()
        )
        # Get the image corresponding to the closest embedding
        image = embeddings.x[closest_embedding_index]
        # Add the image to the list of images
        images.append(image)

    return nearest_neighbors, images
