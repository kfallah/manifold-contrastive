...
class Model(nn.Module):
    ...
    def unshuffle_outputs(
        ...
        header_out = HeaderOutput(
            header_out.prediciton_0, prediction_1, header_out.distribution_data
        )
    ...
    def forward(self, x: torch.Tensor, x_idx: List) -> ModelOutput:
        ...
        if self.model_cfg.enable_backbone_momentum and x.shape[1] == 2:
            header_input, header_out = self.unshuffle_outputs(
    ...
    def update_momentum_network(self, model_out: ModelOutput) -> None:
        ...
        if self.model_cfg.enable_header_momentum:
            self.contrastive_header.update_momentum_network(
                self.model_cfg.header_momentum_update_rate, model_out
            )

    def get_param_groups(self):
        return [
            {"params": self.backbone.parameters()}
        ] + self.contrastive_header.get_param_groups()

    @staticmethod
    def initialize_model(model_cfg: ModelConfig, devices: List[int]) -> "Model":
        backbone, backbone_feature_dim = Backbone.initialize_backbone(
