...
class SparseCodeAttn(nn.Module):
    def __init__(self, cfg: AttentionConfig, dict_size: int):
        ...
        self.attn = nn.MultiheadAttention(cfg.feature_dim, num_heads=cfg.num_heads, batch_first=True)
...
class VIEncoder(nn.Module):
    ...
    def max_elbo_sample(self, log_scale, shift, psi, x0, x1):
        with torch.no_grad():
            ...
            for _ in range(self.cfg.total_num_samples // self.cfg.samples_per_iter):
                ...
                transop_loss = torch.nn.functional.mse_loss(x1_hat, x1.unsqueeze(0).repeat(self.cfg.samples_per_iter, 1, 1, 1), reduction="none").mean(dim=-1)
    ...
    def forward(self, curr_iter, x0, x1, psi):
        ...
        if not self.cfg.variational:
            shift = self.shift(z)
            return shift, torch.tensor(0.), (torch.zeros_like(shift), shift)
...
def init_models(exp_cfg):
    ...
    state_dict = torch.load(exp_cfg.run_dir + f"checkpoints/checkpoint_epoch{current_checkpoint}.pt", map_location=default_device)
    ...
    if exp_cfg.transop_cfg.train_transop:
        psi_init = TransOp_expm(exp_cfg.transop_cfg.dict_size, 64, stable_init=True, 
                           real_range=exp_cfg.transop_cfg.init_real_range, 
                           imag_range=exp_cfg.transop_cfg.init_imag_range).to(default_device)
        psi = psi_init.psi
    else:
...
def train(exp_cfg, train_dataloader, backbone, nn_bank, psi, encoder):
    ...
    if exp_cfg.vi_cfg.opt_type == "SGD":
        opt = torch.optim.SGD(param_list, lr=exp_cfg.vi_cfg.lr, nesterov=True, momentum=0.9, weight_decay=exp_cfg.vi_cfg.weight_decay)
    ...
    scheduler = LinearWarmupCosineAnnealingLR(opt, warmup_epochs=20 * iters_per_epoch, max_epochs=iters_per_epoch*exp_cfg.num_epochs, eta_min=1e-4)
    ...
    for i in range(exp_cfg.num_epochs):
        for idx, batch in enumerate(train_dataloader):
            ...
            if curr_iter % exp_cfg.vi_cfg.logging_interval == 0:
                ...
                wandb.log({
                    'meta/avg_enc_grad': enc_grad.abs().mean(),
                    'meta/max_enc_grad': enc_grad.abs().max(),
                    'meta/med_enc_grad': torch.median(enc_grad.abs())
                }, step=curr_iter)     
            ...
            if curr_iter % exp_cfg.vi_cfg.logging_interval == 0:
                ...
                log.info(f"Iter {curr_iter:6n} -- TO loss: {np.mean(transop_loss_save[-exp_cfg.vi_cfg.logging_interval:]):.3E}," +
